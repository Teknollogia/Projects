...
function EditableTable() {
    ...
    const handleAddRow = () => {
        if (isNaN(newRow.value) || newRow.value === "") {
            alert("A szám mezőnek számot kell tartalmaznia!");
            return;
        }
    ...
    return (
        <div>
                ...
                <tbody>
                    {rows.map(row => (
                        <tr key={row.id}>
                            ...
                            <td>
                                {editingId === row.id ? (
                                    <button onClick={() => handleSave(row.id)}>💾 Mentés</button>
                                ) : (
                                    <button onClick={() => handleEdit(row.id)}>✏️ Szerkesztés</button>
                                )}
                                <button onClick={() => handleDelete(row.id)}>❌ Törlés</button>
                            </td>
                        </tr>
                    ))}
                    <tr id="final_row">
            ...
            <div>
                ...
                <input placeholder="Év" value={newRow.year} onChange={(e) => handleNewRowChange(e, 'year')} />
                <input placeholder="Fejlődési mérföldkő" value={newRow.milestone} onChange={(e) => handleNewRowChange(e, 'milestone')} />
                <input placeholder="Hatás" value={newRow.impact} onChange={(e) => handleNewRowChange(e, 'impact')} />
                <input placeholder="Érték" value={newRow.value} onChange={(e) => handleNewRowChange(e, 'value')} />
